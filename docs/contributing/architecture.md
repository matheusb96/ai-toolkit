...
Three reasons produced this split. The first is the interface each component offers, which produced the packages at the top. One is an import, one is a command, and one is a tool call. The second is the cost of an install, which produced the libraries beneath, so that one install never pays another's dependencies. The third is that a model needs a procedure and a tool description must not carry one, which is `QR-23`, so the procedure ships as a playbook beside the code.

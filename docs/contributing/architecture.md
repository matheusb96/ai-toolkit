...
| LLM agent | A program that runs a model's decisions. It calls an MCP tool, it runs a CLI command in a shell, or it writes a program against the SDK | Per component. SDK: the public surface that the Programmer row names. CLI: a discoverable command set whose output it can parse and pipe into the next call ([`docs/cli`](../cli/README.md)). MCP: the tool descriptions its client gives it ([Tool surface](#tool-surface)). Skills: the playbooks in [`skills/`](../../skills/README.md) |
